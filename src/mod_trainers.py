...
class CoSeRecTrainer(mod_CoSeRecTrainer):
    ...
    def iteration(self, epoch, dataloader, full_sort=True, mode='train'):
        if mode == 'train':
            ...
            for i, (rec_batch, cl_batches) in rec_cf_data_iter:
                ...
                # ---------- recommendation task ---------------#
                sequence_output, aux_loss = self.model(input_ids)
                ...
                joint_loss = self.args.rec_weight * rec_loss + aux_loss*self.aux_weight
            ...
            post_fix = {
                "epoch": epoch,
                "GATRec Contribution": '{:.4f}'.format(self.model.get_moe_weights()[0]),
                "GRU4Rec Contribution": '{:.4f}'.format(self.model.get_moe_weights()[1]),
                "SASRec Contribution": '{:.4f}'.format(self.model.get_moe_weights()[2]),
                "rec_avg_loss": '{:.4f}'.format(rec_avg_loss / len(rec_cf_data_iter)),
                "aux_avg_loss": '{:.4f}'.format(aux_avg_loss / len(rec_cf_data_iter)),
